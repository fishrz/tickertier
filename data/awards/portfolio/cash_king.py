"""💸 钞能力之王 — biggest cumulative dollar gain since cost basis."""
from __future__ import annotations


def compute(con, period_key: str, period: str = "D") -> list[tuple[str, float, dict]]:
    """Σ unrealized $ gain across positions on the snapshot date.

    period_key is unused for D — we always score against the latest snapshot.
    """
    rows = con.execute(
        """
        WITH pos AS (
